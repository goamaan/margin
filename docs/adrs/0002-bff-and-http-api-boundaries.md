...
TanStack Start supports server functions and server routes. Margin also needs durable backend boundaries for compliance, auditability, tenant isolation, external tools, standards, mobile clients, and possible standalone services.

## Decision

Use `apps/api` as Margin's canonical HTTP API runtime from the beginning. Use `packages/api` for API schemas, typed client helpers, and server adapters. The web app should consume this API path rather than private server functions for core LMS workflows.
